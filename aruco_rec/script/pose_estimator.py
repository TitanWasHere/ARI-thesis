...
class PoseService:
    def __init__(self, N):
        ...
        self.srv = rospy.Publisher('/camera/aruco', Image, self.get_camera_pose_callback)
        self.sub = rospy.Subscriber('/camera/camera/color/image_raw', Image, self.image_callback)
    ...
    def pose_estimation(self, frame):
        ...
        if len(corners) > 0:
            ...
            if ARUCOBASE_MARKER_ID in ids:
                ...
                rvec_camera, tvec_camera, _ = cv2.aruco.estimatePoseSingleMarkers(
                    corners[cameraIdx], ARUCO_SIZE, INTRINSIC_CAMERA, DISTORTION_CAMERA)
                tvec_puppet = None
                rvec_puppet = None
                if LOCALIZE_PUPPET and ARUCOPUPPET_MARKER_ID in ids:
                    puppetIdx = ids.tolist().index(ARUCOPUPPET_MARKER_ID)
                    rvec_puppet, tvec_puppet, _ = cv2.aruco.estimatePoseSingleMarkers(
                        corners[puppetIdx], ARUCO_SIZE, INTRINSIC_CAMERA, DISTORTION_CAMERA)
                if SHOW_IMG:
                    frame = cv2.putText(frame, 'id: '+str(ids[cameraIdx]), (int(corners[cameraIdx][0][0][0]), int(
                        corners[cameraIdx][0][0][1])), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
    ...
    def get_camera_pose(self, tvec, quat):
        ...
        rot_matrix = np.matmul(rot_matrix , DEFAULT_ROT)
    ...
    def get_puppet_pose(self, tvec_base, quat_base, tvec_puppet, quat_puppet):
        ...
        rot_matrix_base = np.matmul(R.from_quat(quat_base).as_matrix().reshape(3, 3), DEFAULT_ROT)
        rot_matrix_puppet = np.matmul(R.from_quat(quat_puppet).as_matrix().reshape(3, 3) , DEFAULT_ROT)
        puppet_to_aruco_rot = np.matmul(rot_matrix_base.T , rot_matrix_puppet)
        puppet_to_aruco_pos = np.matmul(rot_matrix_base.T , (tvec_puppet - tvec_base))
        ...
        r = R.from_matrix(np.matmul(puppet_to_aruco_rot , DEFAULT_ROT))
        pose_msg.pose.orientation.x, pose_msg.pose.orientation.y, pose_msg.pose.orientation.z, pose_msg.pose.orientation.w = r.as_quat()
        return pose_msg
    ...
    def get_camera_pose_callback(self, request):
        ...
        while rospy.is_shutdown() and self.mean_tvec is None and i < 200:
            i += 1
        ...
        if LOCALIZE_PUPPET and self.tvec_puppet is not None and self.quat_puppet is not None:
            puppet_pose_msg = self.get_puppet_pose(
                self.mean_tvec, self.mean_quat, self.tvec_puppet, self.quat_puppet)
    ...
    def quaternion_avg(self, quat_list):
        ...
        for i in range(quat_list.shape[0]):
            M = np.matmul(M + 1/self.N * quat_list[i].T, quat_list[i])
        w, v = np.linalg.eig(M)

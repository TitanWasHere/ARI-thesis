...
function displayPOIs(pois) {
    ...
    // Add to container a row with in it an h1 tag
    var row = document.createElement('div');
    row.className = 'row';
    row.style.padding = '10px';
    var h1 = document.createElement('h1');
    h1.innerText = 'Go to POI';
    h1.style.color = '#ed8b00';
    ...
    for (let marker of pois.markers) { // Utilizza for...of per iterare sugli elementi di markers
        ...
        button.addEventListener('click', function() {
            text = "Confermi di voler andare a " + marker.name + "? (Attenzione che ARI partirà subito!)";
            if (confirm(text)) {
                goToPOI(marker.name);
            }
        });
...
function toggleStreamlit() {
    document.getElementById('button_container').style.display = 'none';
    document.getElementById('feed_container').style.display = 'none';
    document.getElementById('POI_container').style.display = 'none';

    // Mostra il contenitore dello Streamlit iframe
    document.getElementById('streamlit_container').style.display = null;

    // Imposta l'URL dell'iframe se non già impostato
    var iframe = document.getElementById('streamlit_frame');
    if (!iframe.src) {
        iframe.src = 'http://192.168.1.113:8501?embed=true' // Sostituisci con l'URL del tuo server Streamlit
    }
}
...
function toggleFeed() {
    ...
    document.getElementById('feed_container').style.display = null;
    document.getElementById('POI_container').style.display = 'none';
    document.getElementById('streamlit_container').style.display = 'none';
}
